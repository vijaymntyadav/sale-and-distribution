...
class MDController extends Controller
{
    ...
    {
        $sales = AllSale::orderBy('id','DESC')->paginate(24);
        return view('admin.md_ed.index',compact('sales'));
    }
    public function saleSearch(Request $request){
        $sales = AllSale::where('em_name','like','%'.$request->search.'%')
                        ->orWhere('position','like','%'.$request->search.'%')
                        ->orWhere('organization_name','like','%'.$request->search.'%')
                        ->orWhere('number','like','%'.$request->search.'%')
                        ->orWhere('total_amount','like','%'.$request->search.'%')
                        ->orWhere('total_outstanding','like','%'.$request->search.'%')
    ...
    public function pending($id){
        ...
        if ($all_sale->sm_approved == 0){
            $all_sale->update([
               'sm_approved' => 1,
               'sm_name' => Auth::user()->name,
               'sm_email' => Auth::user()->email,
            ]);
        }
        if ($all_sale->account_approved == 0){
            $all_sale->update([
               'account_approved' => 1,
        ...
        $all_sale->update([
            'md_approved' => 1,
            'md_name' => Auth::user()->name,
            'md_email' => Auth::user()->email,
            'distributor' => 1,
        ]);
        $sale->update([
            'permission' => '1'
